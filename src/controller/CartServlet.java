	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(type!=null && type.equals("buy")){
			...
			for(int i=0;i<buygoods.size();i++){
				...
				totalprice +=cart.getAmount() * Integer.valueOf(cart.getGsaleprice()).intValue(); 
			}
			java.util.Date orderdate = new java.util.Date();
			Order order =new Order(orderid, null, userid, totalprice, orderdate);
			service.OrderService os =new service.OrderServiceImpl();
			os.insertOrder(order);
			//订单详情;
			service.OrderDetailService ods =new service.OrderDetailServiceImpl();
			for(Cart c:buygoods){
				String orderdetailid = UUID.randomUUID().toString();
				String gtitle = c.getGtitle();
				String gsalprice = c.getGsaleprice();
